...
const Dashboard = () => {
  ...
  const COLORS = ['#004d4d', '#00c853', '#00bfa5', '#0a84ff', '#ff9f0a', '#ff6b6b', '#5856d6', '#af52de'];
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      ...
      // Gender distribution
      setGenderData([
        { name: 'Male', value: basicStats.gender?.male || studentsData.male || 0, color: '#004d4d' },
        { name: 'Female', value: basicStats.gender?.female || studentsData.female || 0, color: '#00c853' }
      ]);
      ...
      // Staff by type
      setStaffByType([
        { name: 'Teachers', value: staffData.teachers || 0, color: '#004d4d' },
        { name: 'Admin', value: staffData.admin || staffData.administrative || 0, color: '#00c853' },
        { name: 'Support', value: staffData.support || staffData.supportive || 0, color: '#00bfa5' }
      ]);
  ...
  const loadSampleData = () => {
    ...
    setGenderData([
      { name: 'Male', value: 256, color: '#004d4d' },
      { name: 'Female', value: 229, color: '#00c853' }
    ]);
      ...
      <div className={styles.chartsGrid}>
        ...
        >
          <div className={styles.chartHeader}>
            <h3><Activity size={18} /> Attendance Trend</h3>
          </div>
          <ResponsiveContainer width="100%" height={280}>
            <AreaChart data={attendanceData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
              ...
              <Tooltip contentStyle={{ background: '#fff', border: 'none', borderRadius: 12, boxShadow: '0 8px 24px rgba(0,0,0,0.12)', fontSize: 12 }} />
              <Legend wrapperStyle={{ fontSize: 11, paddingTop: 16 }} />
              <Area type="monotone" dataKey="present" stroke="#00c853" strokeWidth={2.5} fillOpacity={1} fill="url(#gradPresent)" dot={false} activeDot={{ r: 5, fill: '#00c853', stroke: '#fff', strokeWidth: 2 }} />
              <Area type="monotone" dataKey="absent" stroke="#ff6b6b" strokeWidth={2} fillOpacity={1} fill="url(#gradAbsent)" dot={false} activeDot={{ r: 5, fill: '#ff6b6b', stroke: '#fff', strokeWidth: 2 }} />
            </AreaChart>
          </ResponsiveContainer>
        </motion.div>
        ...
        >
          <div className={styles.chartHeader}>
            <h3><TrendingUp size={18} /> Academic Performance</h3>
          </div>
          <ResponsiveContainer width="100%" height={280}>
            <AreaChart data={performanceData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
              <defs>
                <linearGradient id="gradAvg" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#004d4d" stopOpacity={0.25}/>
                  <stop offset="100%" stopColor="#004d4d" stopOpacity={0.02}/>
                </linearGradient>
              ...
              <Tooltip contentStyle={{ background: '#fff', border: 'none', borderRadius: 12, boxShadow: '0 8px 24px rgba(0,0,0,0.12)', fontSize: 12 }} />
              <Legend wrapperStyle={{ fontSize: 11, paddingTop: 16 }} />
              <Area type="monotone" dataKey="average" stroke="#004d4d" strokeWidth={2.5} fillOpacity={1} fill="url(#gradAvg)" dot={{ r: 4, fill: '#004d4d', stroke: '#fff', strokeWidth: 2 }} activeDot={{ r: 6 }} name="Average" />
              <Area type="monotone" dataKey="passRate" stroke="#00bfa5" strokeWidth={2} fillOpacity={1} fill="url(#gradPass)" dot={{ r: 3, fill: '#00bfa5', stroke: '#fff', strokeWidth: 2 }} activeDot={{ r: 5 }} name="Pass Rate" />
            </AreaChart>
          </ResponsiveContainer>
        </motion.div>
        ...
        >
          <div className={styles.chartHeader}>
            <h3><DollarSign size={18} /> Revenue Overview</h3>
          </div>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={revenueData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }} barGap={4}>
              <CartesianGrid strokeDasharray="3 3" stroke={'#f0f0f0'} vertical={false} />
              <XAxis dataKey="month" stroke={'#9ca3af'} fontSize={11} tickLine={false} axisLine={false} />
              <YAxis stroke={'#9ca3af'} fontSize={11} tickLine={false} axisLine={false} />
              <Tooltip contentStyle={{ background: '#fff', border: 'none', borderRadius: 12, boxShadow: '0 8px 24px rgba(0,0,0,0.12)', fontSize: 12 }} cursor={{ fill: 'rgba(0,77,77,0.04)' }} />
              <Legend wrapperStyle={{ fontSize: 11, paddingTop: 16 }} />
              <Bar dataKey="collected" fill="#00c853" radius={[6, 6, 0, 0]} name="Collected" />
              <Bar dataKey="pending" fill="#ff9f0a" radius={[6, 6, 0, 0]} name="Pending" />
            </BarChart>
          </ResponsiveContainer>
        </motion.div>
        ...
        >
          <div className={styles.chartHeader}>
            <h3><Users size={18} /> Student Distribution</h3>
          </div>
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie
              ...
              <Tooltip contentStyle={{ background: '#fff', border: 'none', borderRadius: 12, boxShadow: '0 8px 24px rgba(0,0,0,0.12)', fontSize: 12 }} />
              <Legend wrapperStyle={{ fontSize: 11, paddingTop: 16 }} formatter={(value, entry) => <span style={{ color: '#6b7280' }}>{value}</span>} />
            </PieChart>
          </ResponsiveContainer>
        </motion.div>
        ...
        >
          <div className={styles.chartHeader}>
            <h3><GraduationCap size={18} /> Class Performance</h3>
          </div>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={classPerformance} layout="vertical" margin={{ top: 10, right: 20, left: 0, bottom: 0 }} barSize={20}>
              <CartesianGrid strokeDasharray="3 3" stroke={'#f0f0f0'} horizontal={false} />
              <XAxis type="number" stroke={'#9ca3af'} fontSize={11} tickLine={false} axisLine={false} domain={[0, 100]} />
              <YAxis dataKey="name" type="category" stroke={'#6b7280'} width={70} fontSize={11} tickLine={false} axisLine={false} />
              <Tooltip contentStyle={{ background: '#fff', border: 'none', borderRadius: 12, boxShadow: '0 8px 24px rgba(0,0,0,0.12)', fontSize: 12 }} cursor={{ fill: 'rgba(0,77,77,0.04)' }} />
              <Bar dataKey="score" fill="#004d4d" radius={[0, 8, 8, 0]} background={{ fill: '#f8faf9', radius: [0, 8, 8, 0] }} />
            </BarChart>
          </ResponsiveContainer>
        </motion.div>
        ...
        >
          <div className={styles.chartHeader}>
            <h3><Award size={18} /> Top Performers</h3>
          </div>
            ...
            {topStudents.map((student, index) => (
              <div key={index} className={styles.topStudentItem}>
                <div className={styles.studentRank} style={{
                  background: index === 0 ? '#004d4d' : index === 1 ? '#00897b' : index === 2 ? '#00bfa5' : '#e8eeec'
                }}>
                  {index + 1}
                </div>
        ...
        >
          <div className={styles.chartHeader}>
            <h3><AlertTriangle size={18} /> Faults by Type</h3>
          </div>
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie
                data={faultsByType.length > 0 ? faultsByType : [{ type: 'No Data', count: 1 }]}
                cx="50%"
                cy="50%"
                innerRadius={65}
                outerRadius={95}
                ...
                stroke="none"
              >
                {(faultsByType.length > 0 ? faultsByType : [{ type: 'No Data', count: 1 }]).map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={faultsByType.length > 0 ? COLORS[index % COLORS.length] : ('#e8eeec')} />
                ))}
              </Pie>
              <Tooltip contentStyle={{ background: '#fff', border: 'none', borderRadius: 12, boxShadow: '0 8px 24px rgba(0,0,0,0.12)', fontSize: 12 }} />
            </PieChart>
        ...
        >
          <div className={styles.chartHeader}>
            <h3><UserCheck size={18} /> Staff Distribution</h3>
          </div>
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie
              ...
              <Tooltip contentStyle={{ background: '#fff', border: 'none', borderRadius: 12, boxShadow: '0 8px 24px rgba(0,0,0,0.12)', fontSize: 12 }} />
              <Legend wrapperStyle={{ fontSize: 11, paddingTop: 16 }} formatter={(value) => <span style={{ color: '#6b7280' }}>{value}</span>} />
            </PieChart>
          </ResponsiveContainer>
        </motion.div>
          ...
          <div className={styles.trafficWidget}>
            ...
            <ResponsiveContainer width="100%" height={100}>
              <BarChart data={attendanceData.slice(0, 7)}>
                <Bar dataKey="present" fill="#004d4d" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
