import { useState, useEffect } from 'react'; import axios from 'axios'; import { FiGrid, FiLoader, FiDownload, FiPrinter, FiCopy } from 'react-icons/fi';
...
export default function Worksheet() {
  ...
  const generate = async () => {
    ...
    try { const res = await axios.post('/api/ai/generate/worksheet', { ...form, schoolConfig, duration: 40, topic: form.subject }); setResult(res.data.data.worksheet); } catch (err) { if (err.response?.data?.type) setError(err.response.data); else setError(err.response?.data?.error || 'Failed'); } finally { setLoading(false); }
  };
  const terms = Array.from({ length: schoolConfig.number_of_terms || 4 }, (_, i) => i + 1);
  return (
    <div className={styles.container}>
      <div className={styles.header}><div className={styles.headerLeft}><div className={styles.headerIcon}><FiGrid /></div><div><h1>Worksheet Generator</h1><p>Generate printable classroom worksheets</p></div></div></div>
      <div className={styles.genGrid}>
        <div className={styles.formCard}>
          ...
          <button className={styles.genBtn} onClick={generate} disabled={loading}>{loading ? <><FiLoader className={styles.spin} /> Generating...</> : 'Generate Worksheet'}</button>
        </div>
        <div className={styles.resultCard}>
          ...
          {result && (<>
            <div className={styles.toolbar}>
              <button className={styles.toolBtn} onClick={() => exportPDF(result.topic||'Worksheet', result)} aria-label="Export PDF"><FiDownload /> Export PDF</button>
              <button className={styles.toolBtn} onClick={() => printContent('ws-output')} aria-label="Print"><FiPrinter /> Print</button>
            ...
            <div className={genStyles.output} id="ws-output">
              ...
              {result.sections?.map((s,i) => (<div key={i} className={genStyles.step}><h3>{s.title}</h3><p><em>{s.type}</em> — {s.instructions}</p>{s.questions?.map((q,j) => <p key={j}><strong>{j+1}.</strong> {q.question} {q.answer && <span style={{color:'var(--color-primary)'}}>→ {q.answer}</span>}</p>)}</div>))}
            </div>
          </>)}
        </div>
      </div>
