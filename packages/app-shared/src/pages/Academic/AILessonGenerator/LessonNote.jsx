...
const LessonNote = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setResult(null);
    try {
      const res = await axios.post('/api/ai/generate-lesson-note', form);
      setResult(res.data.data?.lessonNote || res.data.data);
    } catch (err) {
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: 24 }}>
      ...
      <h1 style={{ fontSize: 24, fontWeight: 700, color: '#1e293b', marginBottom: 4 }}>Lesson Note Generator</h1>
      <p style={{ color: '#64748b', marginBottom: 24 }}>Generate detailed teacher lesson notes using DeepSeek AI</p>
      ...
      {result && (
        <div style={{ background: 'white', borderRadius: 12, padding: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
          <h2 style={{ fontSize: 20, fontWeight: 600, color: '#667eea', marginBottom: 16 }}>{result.topic}</h2>
          ...
          <Section title="Objectives">
            {result.objectives?.map((o, i) => <li key={i} style={liStyle}>{o}</li>)}
          </Section>

          <Section title="Key Concepts">
            {result.keyConcepts?.map((c, i) => (
              <div key={i} style={{ marginBottom: 8 }}>
                <p style={{ fontWeight: 600, color: '#334155' }}>{c.term}</p>
                <p style={{ color: '#475569' }}>{c.definition}</p>
              </div>
            ))}
          </Section>

          <Section title="Main Explanation">
            <div style={{ color: '#475569', lineHeight: 1.8, whiteSpace: 'pre-wrap' }}>{result.mainExplanation}</div>
          </Section>

          <Section title="Examples">
            {result.examples?.map((ex, i) => (
              <div key={i} style={{ marginBottom: 12, padding: 12, background: '#f8fafc', borderRadius: 8 }}>
                <p style={{ fontWeight: 500, color: '#334155' }}><strong>Problem:</strong> {ex.problem}</p>
                <p style={{ color: '#475569', marginTop: 4 }}><strong>Solution:</strong> {ex.solution}</p>
              </div>
            ))}
          </Section>

          <Section title="Classroom Activities">
            {result.classroomActivities?.map((a, i) => (
              <div key={i} style={{ marginBottom: 12, padding: 12, background: '#f0f9ff', borderRadius: 8 }}>
                <p style={{ fontWeight: 600, color: '#0369a1' }}>{a.activity} ({a.duration})</p>
                <p style={{ color: '#475569', marginTop: 4 }}>{a.instructions}</p>
              </div>
            ))}
          </Section>

          <Section title="Important Notes">
            {result.importantNotes?.map((n, i) => <li key={i} style={liStyle}>⚠️ {n}</li>)}
          </Section>

          <Section title="Summary">
            <p style={{ color: '#475569', lineHeight: 1.6, fontStyle: 'italic' }}>{result.summary}</p>
          </Section>

          <Section title="Review Questions">
            {result.reviewQuestions?.map((q, i) => (
              <div key={i} style={{ marginBottom: 8, padding: 8, borderLeft: '3px solid #667eea', paddingLeft: 12 }}>
                <p style={{ color: '#334155' }}><strong>Q{i + 1}:</strong> {q.question}</p>
                <p style={{ color: '#64748b', fontSize: 13, marginTop: 2 }}><strong>Answer:</strong> {q.expectedAnswer}</p>
              </div>
            ))}
          </Section>
